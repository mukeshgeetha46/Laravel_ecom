					...
					<div class="wrap-product-detail">
						...
						<div class="detail-info">
							<div class="product-rating">
								...
								@foreach ($product->orderItems->where('rstatus',1) as $orderItem)
								  @php
									  $avgrateing = $avgrateing + $orderItem->review->rating;
								  @endphp
								@endforeach
								@for ($i = 1; $i <=5; $i++)
							...
							@if($product->sale_price > 0 && $sales->status == 1 && $sales->sale_date > Carbon\Carbon::now())
                            ...
                            <div class="quantity" style="margin-top: 10px;">
								...
								<div class="quantity-input">
									<input type="text" name="product-quatity" value="1" data-max="120" pattern="[0-9]*" id="product-quatity">
							...
							<div class="wrap-butons">
								@if($product->sale_price > 0 && $sales->status == 1 && $sales->sale_date > Carbon\Carbon::now())
								<a href="#"  class="btn add-to-cart" onclick="store({{$product->id}},{{$product->name}},{{$product->sale_price}})">Add to Cart</a>
                                @else
								<a href="#" class="btn add-to-cart" onclick="store({{$product->id}},'{{$product->name}}',{{$product->regular_price}})">Add to Cart</a>
								@endif
						...
						<div class="advance-info">
							<div class="tab-control normal">
								...
								<a href="#add_infomation" id="add_infomationtab" class="tab-control-item">Addtional Infomation</a>
								<a href="#review" id="reviewtab" class="tab-control-item">Reviews</a>
							</div>
...
<div class="wrap-show-advance-info-box style-1 box-in-site">
	...
	{{-- <script>
		$(document).on('click','#add_infomationtab',function(e) {
    // use this if you want limit clicks in .directFilter only
    // $(document).on('click','.directFilter',function(e)

    $this = $(e.target);
	if($this.text().trim()=='Addtional Infomation'){
		$('.owl-prev').css('margin-bottom','80px');
	}else{
		$('.owl-prev').css('margin-top','84px');
	}
    alert($this.text())
});
	</script> --}}
