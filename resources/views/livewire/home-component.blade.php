<main id="main">
		<div class="container">
			...
			<!--On Sale-->
			@if($sproducts->count() > 0 && $sales->status == 1 && $sales->sale_date < Carbon\Carbon::now())
				...
				<div class="wrap-products">
					<div class="wrap-product-tab tab-style-1">
						<div class="tab-control">
							...
							<a href="#category_{{$categorie->id}}" class="tab-control-item {{$key==0 ? 'active':''}}">{{$categorie->name}}</a>
							@endforeach
						</div>
						<div class="tab-contents">
							@foreach($categories as $key=>$categorie)

							<div class="tab-content-item {{$key==0 ? 'active':''}}" id="category_{{$categorie->id}}">
								<div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" >
									@php
									  $c_products = DB::table('products')->where('category_id',$categorie->id)->get()->take($no_of_product);
									@endphp
